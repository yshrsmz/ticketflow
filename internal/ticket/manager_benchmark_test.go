...
// BenchmarkManagerListSequentialVsConcurrent compares sequential vs concurrent list implementations
func BenchmarkManagerListSequentialVsConcurrent(b *testing.B) {
	scenarios := []struct {
		name        string
		ticketCount int
	}{
	...
	for _, scenario := range scenarios {
		b.Run(scenario.name, func(b *testing.B) {
			...
			dirs := []string{todoDir, doingDir, doneDir}
			...
			b.Run("sequential", func(b *testing.B) {
				...
				b.ResetTimer()
				for i := 0; i < b.N; i++ {
					_, err := manager.listSequential(ctx, dirs)
					if err != nil {
						b.Fatal(err)
					}
				}
			...
			b.Run("concurrent", func(b *testing.B) {
				...
				b.ResetTimer()
				for i := 0; i < b.N; i++ {
					_, err := manager.listConcurrent(ctx, dirs)
					if err != nil {
						b.Fatal(err)
					}
				}
